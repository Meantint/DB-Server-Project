...
- Server와 Client의 소켓 통신 구현

- 파일 입출력 기능을 이용해 DB 관리

- 정해진 명령어(connect, save, read, clear, exit)로 클라이언트에서 서버에 저장된 DB 접근

  - connect `IP` : 해당 `IP` 주소로 연결을 요청한다.

  - save key:value : 해당 key가 존재하는 경우 value 값을 갱신하고 존재하지 않는 경우 새로운 key와 value를 생성한다.

  - read key : 해당 key에 맞는 value를 불러온다.

  - clear : DB 데이터를 초기화한다.

  - exit : server에게 종료 메시지를 보낸다.
